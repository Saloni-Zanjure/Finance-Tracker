...
def dashboard_view(request):
    # Fetch total income for the logged-in user
    total_income = UserIncome.objects.filter(owner=request.user).aggregate(total=Sum('amount'))['total'] or 0.00

    # Fetch total expenses for the logged-in user
    total_expenses = Expense.objects.filter(owner=request.user).aggregate(total=Sum('amount'))['total'] or 0.00
...
class RegistrationView(View):
    ...
    def post(self, request):
        ...
        user = User.objects.create_user(username=username, email=email)
        user.set_password(password)
        user.is_active = True  # Make the user active immediately, no email required
        user.save()
...
#this is the login view of the code 
...
class RequestPasswordResetEmail(View):
    def get(self, request):
        return render(request, 'authentication/reset-user-password')
...
class CompletePasswordReset(View):
    ...
    def post(self, request, uidb64, token):
        ...
        try: 
            ...
            user= User.objects.ge (pk=user_id)   #here it is used force_text
